...
description: Redesenha sites de leads prospectados no Prospector de Sites. Use quando o usuario quiser redesenhar site, melhorar pagina, refazer site do cliente, gerar pagina premium, criar editor visual ou atualizar comparador antes/depois.
---
...
8. **Comparador sempre.** Todo lote de redesign termina com `comparar.html` na raiz da pasta de trabalho, gerado a partir de `references/comparador-template.html` (substituir `__CLIENTES__` pelo array JSON; mesclar com clientes já existentes). A entrega padrão de cada cliente são 3 arquivos: página + editor + aba no comparador.
...
A camada de edição visual (para gerar `[slug]-editor.html`) está em `references/editor-visual.md` — injetar o script exatamente como documentado lá ou usar `scripts/inject-editor.py` quando disponivel. O comparador antes/depois está em `references/comparador-template.html` — substituir `__CLIENTES__` pelo array JSON e salvar como `comparar.html` na raiz da pasta de trabalho, ou usar `scripts/update-comparator.py` para mesclar com clientes existentes.
